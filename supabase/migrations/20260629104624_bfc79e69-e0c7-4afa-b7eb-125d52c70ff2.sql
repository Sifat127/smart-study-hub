...
DECLARE
  ...
  force_was_on boolean;
BEGIN
  -- Remember the current FORCE RLS setting and turn it on so the table owner
  -- (this SECURITY DEFINER function) is also subject to the policies.
  SELECT relforcerowsecurity INTO force_was_on
  FROM pg_class WHERE oid = 'public.profiles'::regclass;
  ALTER TABLE public.profiles FORCE ROW LEVEL SECURITY;

  -- Create two real auth users (handle_new_user trigger inserts their profile + role rows).
  ...
  --------------------------------------------------------------------
  -- Restore FORCE setting, verify Bob, clean up.
  --------------------------------------------------------------------
  IF NOT force_was_on THEN
    ALTER TABLE public.profiles NO FORCE ROW LEVEL SECURITY;
  END IF;

  -- Cleanup needs to read Bob's row across RLS-as-owner; FORCE is back to its previous state.
